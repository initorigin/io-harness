...
| 0.6.0 | A developer runs a task whose generated code executes inside an ephemeral local sandbox that is created, run, captured, and destroyed per run, so execution-based verification and any agent-run command no longer compile and run model-produced code directly on the host — closing the "compiles locally, no isolation" limitation carried since 0.2.0 and made sharper by 0.5.0's many concurrent agents. | outline | planned |
| 0.7.0 | A developer starts a task and leaves it: the harness runs unattended for 24h+ with no user input, checkpointing progress so a crash or restart resumes the whole agent tree where it stopped, pausing for a human only when the policy demands it and continuing once a decision arrives. | hypothesis | planned |
