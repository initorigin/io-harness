...
pub enum EventKind {
    ...
    /// driven and before the lease is taken.
    ///
    /// The pause is also the run's outcome
    /// ([`RunOutcome::AwaitingRecovery`](crate::RunOutcome::AwaitingRecovery));
    /// this is here so a caller driving many runs learns which attempt is holding
    /// which run without opening the store.
    RecoveryPaused {
        /// The journal row a decision must name.
        attempt_id: i64,
