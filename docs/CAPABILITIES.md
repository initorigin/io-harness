...
| [Observability and replay](guide/observability.md) | Observers, event kinds, outcome records, per-step latency attribution, deterministic replay |
...
| [Accounting](guide/accounting.md) | One row per provider call, the cache and reasoning breakdown, the achieved cache hit rate and the wire that cannot report half of it, latency and TTFT, spend by model, day, run or session, and cost derived from a price table you own |
...
| [CodeAct](guide/codeact.md) | Writing one contained Python program instead of a chain of tool calls, with every act it takes re-entering the same dispatch a model's own call takes — the host interpreter it is handed to, the tools it may not call, the workdir that is the only thing it may write, and the two bounds that stop a program spending the harness's own time |
...
| --- | --- | --- |
| 0.83.0 | **A widened sandbox widens, and a contained child cannot read the operator's keys.** `sandbox.allow_network = true` reached the macOS backend on no real run: the profile's proxied arm matched `(Some(addr), _)` and discarded the flag, and every real run is proxied — so 0.80.0's fix for exactly this report had never executed once. A widened proxied run now grants `network-bind`, which appears nowhere in this crate before this release, so a dev server can serve a port inside the boundary; a narrow one is byte-identical to 0.82.0. On a proxied run only `[sandbox] allow_network` widens, because `permits_any_egress` is true for any allow rule naming any host and the per-host rules are the proxy's to enforce. The prompt's proxied branch reads the same input instead of computing an answer and ignoring it. **Neither defect was catchable**, because no test had ever run a provider that names an endpoint, so no test run had ever been proxied: the mock gains an endpoint, which is the release's load-bearing change. Beside them the two residuals 0.74.0 named and did not close, both live on the common path — the harness's provider keys are scrubbed from every contained child at the one site every backend's spawn converges on, with a contract declaration as the way back, and the browser child gets a temporary directory inside the profile it owns rather than the whole system temporary directory, where every run's workspace lives. Then the disclosure this crate owed: advisory GHSA-p4qf-q7x8-68m8 for 0.74.0's four criticals, and a `SECURITY.md` whose reporting channel is real rather than a placeholder. Carried alongside: `git_log` answers in its own words on a repository with no commits, speculation asks whether a call may run beside its siblings rather than reading `ToolEffect` alone, and `docs/guide/durable-runs.md` names the category it is in | [2026-09-07](../CHANGELOG.md#0830---2026-09-07) |
| 0.82.0 | **A run assembles under the window of the model it is actually asking, on every provider this crate ships.** 0.81.0 built that whole path and shipped it inert: `resolve_budget`, `ContextBudget::for_window` and `EventKind::ContextCeiling` were correct and reachable, and nothing answered `Provider::context_window` — of the four providers only `Compatible` implemented it, and only from a catalogue the embedder had to know to fetch by calling `models()` first, a requirement discoverable by reading the method body. So every run on `OpenRouter`, `Anthropic` and `OpenAi` took the fallback rung. `Provider::warm_sizing` is the async half the two synchronous methods were waiting on: called once before the first step, skipped when the contract stated its own budget or the provider already knows its window, defaulting to no request at all so every existing implementation and every test mock dials nothing, and swallowed on failure because an unreachable catalogue is a guessed ceiling rather than a dead run. `OpenRouter` reads its own `/models`, which is the same document as `catalog::DEFAULT_REFERENCE_URL` and sits on the host it already dials, so no opt-in and no new host. `Anthropic` and `OpenAi` publish identifiers and no window at all, so they gain `with_reference_catalogue`, **off by default and reaching nothing when off** — when set, the reference host joins `Provider::endpoints` and is authorised against the policy's network rules before the first step, so a policy denying it refuses the run rather than skipping the lookup, and the warm happens after that authorisation rather than before it. **The fallback rung stops being one flat number**: `Provider::assumed_window` splits it into `FALLBACK_WINDOW` at 128,000 for a remote endpoint and `FALLBACK_WINDOW_LOCAL` at 24,000 for a loopback base, both sized through `ContextBudget::for_window` so an assumed window reserves the answer and the request floor exactly as a read one does. **The consequence is a spend change and is stated as one**: a run that used to trim at 24,000 now sends what the model can hold — on a 128,000-token model the ceiling moves to 111,616 — for larger prompts against fewer re-reads and less lost history, with `[run.context] max_tokens` unchanged and still winning over everything | [2026-09-06](../CHANGELOG.md#0820---2026-09-06) |
| 0.81.0 | The harness measures itself, and then spends the measurement. `io_harness::eval` is an in-crate evaluation suite over recorded replay — a `Case` is a task contract plus the outcome that decides it, a `Scorer` turns a finished run into one number, a `Suite` runs the cases and reports both, and none of it needs a key or opens a socket. Four scorers: what a request costs whole including the tool array, whether the facts a case declared it needs survived into the prompt, what the catalogue alone costs, and what fraction of injected dangerous acts an approver refused. **It scores the request, not the answer**, and says so: the replay key is the request's own bytes, so no recording can say what a model would have replied to a differently compacted history — an ignored live arm behind an API key carries the questions that genuinely need a model. Deterministic replay has shipped since 0.12.0 with nothing on top of it, which is why Context Collapse, the tool mask and CodeAct are all live and unmeasured; `docs/MEASUREMENTS.md` now carries their numbers, and **no default changed**, because on the case set measured only one rung moved the figure and it moved it 1.4%. Beside the suite, the levers it exists to argue about. Three compaction rungs between Collapse and a fold — reduce, snip, microcompact — all off by default, plus a skill body that folds out of the conversation after the step that used it, and `[run] compaction`, `[run] collapse` and `[run] ladder`, so an operator can choose one from a config file for the first time. Tiered tool exposure: the core file, exec and git tools always offered, `documents`, `browser` and `shell_jobs` named in one line and reached through one `expand_tools` call, a description budget gating every built-in at 60 tokens, and a bundle that says which of its skills are worth a catalogue line. **The context ceiling follows the model** — `ContextBudget::default`'s 24,000 was every run's ceiling on every model because nothing read the window the catalogue already carried — and cached tokens reach the event stream, so a footer stops reporting every re-sent catalogue as paid at full price. `EventKind::ImageAttached` closes an observability gap standing since 0.72.0: no image reached the stream in any form, and the descriptor now does while the bytes still do not. A run can declare writable roots beyond its workdir, which is what finally lets **the Landlock rung stop granting the whole system temporary directory** — the residual 0.74.0 named, 0.80.0 attempted and withdrew because a worktree child could not commit. And `bundles/long-horizon/` is the repository's first capability bundle: a real protocol shipped as skills over primitives that already exist, adding no crate surface | [2026-09-06](../CHANGELOG.md#0810---2026-09-06) |
| 0.80.0 | No capability — the release that fixed everything known to be wrong before anything new shipped. Two defects an io-cli field test found in its first hour: `sandbox.allow_network` and `policy.defaults.net` were documented as the levers that widen a run's network and reached no backend at all, because the containment's egress answer was *replaced* by the policy's rather than combined with it at every spawn site — so no package install could resolve a host — and the dev-server half of that was **not** in fact fixed until 0.83.0, because the macOS profile's proxied arm discarded the flag and every real run is proxied — and the prompt described a containment no command was given, telling the model outbound was scoped by per-host rules that do not bind a contained command in either direction. Beside them the seven security residuals 0.74.0 named and did not close, and the seven more it recorded while closing the fifty-one it did: the Landlock rung's grant of the whole system temporary directory, where every run's ephemeral workspace lives, so two concurrent runs could rewrite each other's from inside their own sandboxes; the absolute-target relaxation that skipped the containment check for every consumer to serve one; provider endpoints resolved a second time after being graded; a skills walk that followed a symbolic link out of its root and put a stranger's frontmatter in the system prompt; a browser child spawned with no containment while every sibling had it, and its page's subresources ungated; an approver's *narrowing* of a host or a redirect discarded in silence at the two sites shaped like the one M4 already refused; an uncapped whole-file read on two of three entry points; and a directory swapped for a symbolic link between the check and the write, followed by every writing entry point in the crate. **`MOUNT_SETUP` ran for the first time on any machine** — three shipped fixes live in that shell and the suite that covers it returned early on every host this crate has ever run on, so a CI leg makes Landlock unavailable at the *syscall* rather than selecting a rung, because a rung selected by configuration is a downgrade an attacker can reach. Issue #246's pair, where a step carrying gate feedback silently lost its assistant turn and its tool-call blocks on the wire; issue #255, where the local Linux cross-check gate had not compiled since 0.74.0 and reported its own skips as a pass. And the security advisory 0.74.0 owed for four criticals on a published crate, drafted against the shipped code rather than the finding text | [2026-09-05](../CHANGELOG.md#0800---2026-09-05) |
| 0.79.1 | No capability — the release that named the ones already shipped. Structured output and `OutputSchema`, `run_program`, `ToolMask` and `Collapse` had all reached users without reaching the README, because every drift checker in this repository catches a documented sentence that became false and none of them catches a capability that was never documented. The README now names each in the section that already covers it, `docs/CONTRACT.md` states what an output schema guarantees and what it cannot, and the asymmetry itself is closed by a gate: every row of this table at or after 0.73.0 must be represented in the README by a name a reader can search for, or be listed as introducing none, with its reason. Beside it, the comparison tables were re-read against each project's own current documentation — five cells had moved, one of the compared projects having removed its sandbox outright — and their stated date now carries a 180-day bound, because it had not moved in nineteen releases | [2026-09-05](../CHANGELOG.md#0791---2026-09-05) |
| 0.79.0 | A sequence of tool calls collapses into one contained program. Behind `codeact`, off by default and adding no crate, `run_program` takes a complete Python program as its argument — source, not a command line — and the tools the run already has are ordinary functions inside it, taking the same arguments by keyword and returning a value that is falsy when the act was refused or failed, so a program branches instead of stopping. A program is a tool call inside one ordinary step, which is what keeps the transcript pairing, the step cap, the attribution columns, the tool mask and the `before_tool` hooks working unchanged. **Every act it takes re-enters the same `dispatch` a model's own call takes** — same policy, same gate, same `policy_events` row, same journal attempt, same `Observer` event — so a program reaches nothing the model could not have reached by asking one call at a time; what collapses is the number of provider round trips, not the number of boundaries. Starting the interpreter is itself an `Act::Exec` check, on the interpreter and on its whole argv, taken before anything is spawned, and the tool is refused while a plan is still waiting for approval — for the reason `remember` is. `Sandbox::run` closes stdin and runs to completion, so a child that calls back cannot go through it: the program is contained the way `shell_start` contains a living child, which selects the same backends under the same rules — and applies none of them on Windows, so a Windows program that asked to be contained is refused by name rather than started under a boundary the trace claims and the process does not have. It runs in an ephemeral workdir of its own rather than in the workspace, with egress denied and no proxy of its own — worth exactly what the backend delivers, which for the portable floor is nothing, and nothing at all on a run that asked for no containment — and the shim takes the protocol descriptors before the program runs one instruction, so nothing a program *prints* can reach or forge the pipe. The interpreter is the host's, resolved from `[codeact]` or from `CODEACT_CANDIDATES` and version-probed against `CODEACT_MIN_PYTHON` so a `python` answering 2.7 is rejected by number rather than trusted by name; **nothing is downloaded, ever**, and a host with no usable one is a supported host — the tool is not offered, the turn runs as it did at 0.78.0, and `EventKind::Program` records which of the two happened. `CODEACT_UNCALLABLE` names what a program may not call, as a literal rather than a derivation, so a built-in added later fails a test instead of becoming callable silently; `max_callbacks` and `timeout` bound what a program makes the harness spend, which is what a tight callback loop exhausts and what no rlimit can see | [2026-09-04](../CHANGELOG.md#0790---2026-09-04) |
| 0.78.0 | The best trace in the field becomes readable by things that are not this crate, and the boundary becomes borrowable by other agents. Behind `otel`, a run exports as OpenTelemetry spans over OTLP/HTTP with a JSON body, following the GenAI semantic conventions: an `invoke_agent` span per run, one per committed step, an `execute_tool` span per tool call, and a `chat <model>` span per `provider_calls` row carrying the model, the token split and the latency that call cost. `OtelExporter` is an `Observer` and nothing else, so the run loop does not move — and it opens **its own** `Store` because the event channel cannot answer this: there is no provider-call event, `EventKind::ToolCall` has no end and no call id, and `RunEvent` carries no timestamp. A provider span's duration is its row's `latency_ms` and its position is derived from the step span the exporter timed itself, because `provider_calls.at` is one-second resolution and stamped after the call. It sends structure and numbers and **never transcript content** — the convention's three opt-in content attributes are not implemented at all rather than defaulted off, and the exporter never receives a goal, a tool target, an argument or an output to begin with. An export failure cannot change a run: `event` returns `Flow::Continue` on every path including a poisoned lock and an absent runtime, the POST leaves the run's task through `Handle::try_current`, and retries follow OTLP's own rules. `[otel]` is refused at project scope, because an export endpoint is an outbound channel and the headers beside it are a credential. Behind `mcp-server`, this crate serves its own tools over MCP on stdio — newline-delimited JSON-RPC 2.0 written here, so `rmcp` stays a client-side dependency — with every served call routed through the same `dispatch` a model's call takes, so the policy gate decides it, a `policy_events` row records it and the journal keeps it. There is no human at the far end of a pipe, so the default approver is `DenyAll` and an asking rule refuses rather than waits; the three tools the policy deliberately does not see are not served at all. Neither feature adds a single package to the dependency graph | [2026-09-04](../CHANGELOG.md#0780---2026-09-04) |
| 0.77.0 | The transcript says what kind of thing each part of it is, and a caller can demand a shape. `Origin` records where every observation's bytes came from — an operator's instruction, the agent's own words, the harness's prose, or external content by source: a file, a shell, a web page, an MCP or LSP server, a skill body, a child agent — set at the construction site rather than inferred, and required by `Observation::new` so a site that forgets is a compile error rather than a silent default. It is stored in a nullable `ledger_observations.origin` column added by `ALTER TABLE` with `CHECKPOINT_FORMAT` unmoved, proven additive in both directions against a real 0.76.0 binary from crates.io, and a row written before this release reads back `Unmarked` rather than as an error or a guess. It rides the `Observer` channel on `EventKind::ToolCall`, so provenance is answerable without opening the database, and it survives an archive because the column is named in `is_fact_column`, which defaults to clearing. External content is then wrapped in an `external_content` tag in the prompt itself, for every prompt family rather than only Anthropic, because a boundary made of prose alone can be forged by the quoted content it is meant to contain — which is the middle of the field's three-part answer to prompt injection, the boundary having shipped in 0.4.0 and neutralization at the subagent boundary being named for a later release. `Origin` and `Piece` stay independent on purpose: a `Piece` is a layout role deciding which entries occupy a tool call's position, and deriving it from provenance would force an operator's own answer to `ask_question` to be recorded as tool output. Alongside it, `OutputSchema` and `TaskContract::with_output_schema` let a caller demand a JSON Schema for a run's final output — validated against a stated, closed keyword subset whose refusal names the offending keyword at declaration time, carried to the OpenAI-shaped wire as `response_format` with `strict: false` because OpenAI's strict subset rejects six keywords this crate validates in full, and enforced locally regardless of what any vendor did with it. A failure re-prompts the model with the validation error as an ordinary ledger observation, bounded by `max_retries`, with every attempt an ordinary step drawing against the run's budgets; exhausting the attempts is `RunOutcome::SchemaUnsatisfied` rather than a finish over malformed output | [2026-09-03](../CHANGELOG.md#0770---2026-09-03) |
| 0.76.0 | A turn can withhold a tool without moving the catalogue, and an observation that will not fit whole can be carried shortened rather than thrown away. `ToolMask` and `TaskContract::with_tool_mask` name the tools a turn may not call; the definitions sent are byte-identical to an unmasked run's, because the tool array sits ahead of 0.38.0's breakpoint at the end of the system block and any byte changed in it invalidates that cache entry and everything after it in the same ordering — so masking withholds availability rather than membership, and the saving is a cached prefix that survives rather than tokens removed. The withheld names are stated after the observation section, past both breakpoints, so a mask that changes between turns costs no cache entry; a call to one of them is refused at both places a call can begin, because a batched read-only call does not route through `dispatch`. `Collapse` and `TaskContract::with_collapse` add the rung beneath a fold: an entry that will not fit the turn's budget is carried shortened rather than replaced by a one-line stub, at no provider call and no `summaries` row, leaving the ledger exactly as long as it was — so turning it off assembles those entries whole again, which a fold cannot do, and a shortened entry keeps its `ObsKind` and `target` so the stale-read rules can still see it. Both are off unless configured, and `fold` stays the last rung and the default trigger. Beside them, six tests that reported a slow CI runner as a broken product and two that would have passed over a genuinely leaked socket are made to wait on states rather than clocks | [2026-09-03](../CHANGELOG.md#0760---2026-09-03) |
| 0.75.0 | A slow or expensive run is diagnosable from the trace, and the two costs the crate paid silently stop being silent. The cache counters recorded since 0.18.0 are finally divided by something — `Usage::cache_hit_rate` and `Spend::cache_hit_rate` — and `cache_write_tokens` becomes an `Option`, because the OpenAI wire reports no cache-write counter at all and recording that absence as a zero made a rate read as a complete accounting of a call that also paid to write. Each committed step records where its wall clock went — the provider, tool execution, the policy gate of which, and the write that ended the step before — inside the same transaction that commits the step, announced as `EventKind::StepAttributed` and read back with `Store::step_attributions`. A session's spend is one query rather than a fold over its turns. Each memory entry's normalised token sets are kept beside it instead of being recomputed for every scope on every turn, which takes a 4,096-entry ranking from 119 ms to 29 and makes `max_entries` a knob an operator can raise. `list_dir` and the git readers join the calls that overlap and start before a completion settles, which 0.41.0 deferred pending exactly this evidence. And `Routing` gains both a `[routing]` config section — refused at project scope, because a cloned repository must not choose which model answers — and `mechanical`, naming the model for the one completion this crate makes on its own behalf | [2026-09-02](../CHANGELOG.md#0750---2026-09-02) |
...
| 0.70.0 | An MCP server or a plugin bundle is switched off without being deleted — `enabled` defaults to true on both, a disabled thing contributes nothing and stays listed as configured-and-off, and a misspelling of that one key inside `[[mcp]]` is caught even though the table is exempt from unknown-key rejection — a configured server can be probed to see whether it actually answers, a session's age and a run's own directory are readable, a run that failed its criterion says so instead of reporting a step cap, a failing gate tells the next step what it printed, and an asking posture on `Act::Exec` raises an approval instead of behaving as a denial | [2026-08-29](../CHANGELOG.md#0700---2026-08-29) |
