...
| --- | --- | --- |
| 0.79.0 | A sequence of tool calls collapses into one contained program. Behind `codeact`, off by default and adding no crate, `run_program` takes a complete Python program as its argument — source, not a command line — and the tools the run already has are ordinary functions inside it, taking the same arguments by keyword and returning a value that is falsy when the act was refused, so a program branches on a refusal instead of stopping. A program is a tool call inside one ordinary step, which is what keeps the transcript pairing, the step cap, the attribution columns, the tool mask and the `before_tool` hooks working unchanged. **Every act it takes re-enters the same `dispatch` a model's own call takes** — same policy, same gate, same `policy_events` row, same journal attempt, same `Observer` event — so a program reaches nothing the model could not have reached by asking one call at a time; what collapses is the number of provider round trips, not the number of boundaries. `Sandbox::run` closes stdin and runs to completion, so a child that calls back cannot go through it: the program is contained the way `shell_start` contains a living child, which selects the same backends under the same rules and inherits that seam's refusal of Windows access confinement rather than claiming a containment it does not have. It runs in an ephemeral workdir of its own rather than in the workspace, with no network and no proxy of its own, and the shim takes the protocol descriptors before the program runs one instruction — so a program that prints ordinary text, writes raw bytes past `sys.stdout`, or prints a well-formed callback frame can neither reach nor forge the pipe. The interpreter is the host's, resolved from `[codeact]` or from `CODEACT_CANDIDATES` and version-probed against `CODEACT_MIN_PYTHON` so a `python` answering 2.7 is rejected by number rather than trusted by name; **nothing is downloaded, ever**, and a host with no usable one is a supported host — the tool is not offered, the turn runs as it did at 0.78.0, and `EventKind::Program` records which of the two happened. `CODEACT_UNCALLABLE` names what a program may not call, as a literal rather than a derivation, so a built-in added later fails a test instead of becoming callable silently; `max_callbacks` and `timeout` bound what a program makes the harness spend, which is what a tight callback loop exhausts and what no rlimit can see | [2026-09-04](../CHANGELOG.md#0790---2026-09-04) |
