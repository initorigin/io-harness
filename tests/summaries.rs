...
fn a_summary_round_trips_through_the_store() {
    ...
    let found = store.summary_at(run_id, 12).unwrap().expect("a row at 12");
...
#[test]
fn a_boundary_is_the_key_and_a_neighbouring_step_is_not_it() {
    let store = Store::memory().unwrap();
    let run_id = store.start_run("port the parser", "openrouter").unwrap();
    store.put_summary(run_id, 12, 40, "the fold at twelve", 4).unwrap();

    assert!(
        store.summary_at(run_id, 11).unwrap().is_none(),
        "a step before the boundary has no summary"
    );
    assert!(
        store.summary_at(run_id, 13).unwrap().is_none(),
        "a step after the boundary is a different boundary, not a fallback to this one"
    );
}
...
fn a_corrected_fold_reads_back_as_the_correction() {
    ...
    assert_eq!(
        store.summary_at(run_id, 12).unwrap().unwrap().text,
        "second attempt",
...
fn a_run_that_never_folded_has_no_summaries() {
    ...
    assert!(store.summary_at(run_id, 1).unwrap().is_none());
}
